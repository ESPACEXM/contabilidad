...
@section('title', 'Nueva Póliza Contable')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select id="type" name="type" required
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                        <option value="diario" {{ old('type') === 'diario' ? 'selected' : '' }}>Diario</option>
                        <option value="ingresos" {{ old('type') === 'ingresos' ? 'selected' : '' }}>Ingresos</option>
                        <option value="egresos" {{ old('type') === 'egresos' ? 'selected' : '' }}>Egresos</option>
                        <option value="apertura" {{ old('type') === 'apertura' ? 'selected' : '' }}>Apertura</option>
                        <option value="cierre" {{ old('type') === 'cierre' ? 'selected' : '' }}>Cierre</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="2"
                              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
