...
<div class="max-w-4xl mx-auto">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <form action="{{ route('chart-accounts.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="type" name="type" required
                        ...
                        <option value="activo" {{ old('type') == 'activo' ? 'selected' : '' }}>Activo</option>
                        <option value="pasivo" {{ old('type') == 'pasivo' ? 'selected' : '' }}>Pasivo</option>
                ...
                <div>
                    ...
                    <select id="nature" name="nature" required
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                        <option value="deudora" {{ old('nature', 'deudora') == 'deudora' ? 'selected' : '' }}>Deudora</option>
                        <option value="acreedora" {{ old('nature') == 'acreedora' ? 'selected' : '' }}>Acreedora</option>
                    </select>
                    @error('nature')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="level" name="level" required
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                        <option value="cuenta_mayor" {{ old('level', 'cuenta_mayor') == 'cuenta_mayor' ? 'selected' : '' }}>Cuenta Mayor</option>
                        <option value="subcuenta" {{ old('level') == 'subcuenta' ? 'selected' : '' }}>Subcuenta</option>
                        <option value="auxiliar" {{ old('level') == 'auxiliar' ? 'selected' : '' }}>Auxiliar</option>
                    </select>
                    @error('level')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                              placeholder="Descripción de la cuenta...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="initial_balance" name="initial_balance" value="{{ old('initial_balance', 0) }}" step="0.01" min="0"
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                ...
                <div class="md:col-span-2">
                    <div class="flex items-center space-x-6">
                        <label class="flex items-center">
                            <input type="checkbox" name="allows_movements" value="1" {{ old('allows_movements', true) ? 'checked' : '' }}
